  ...
  if (shouldShowOverlay) {
    return (
      <div className={`relative ${className}`}>
        ...
        {/* Lock Overlay */}
        <div 
          className="absolute inset-0 bg-black/70 backdrop-blur-[4px] rounded-lg flex items-center justify-center z-[5] cursor-pointer"
        ...
        >
          <div className="text-center text-white p-4 select-none">
            <div className="flex justify-center mb-3">
              <div className="w-16 h-16 bg-gray-800/80 backdrop-blur-sm rounded-full flex items-center justify-center">
                {isLocked && !isUnlocked ? (
                  <FiLock size={28} className="text-yellow-400" />
                ) : (
                  <FiUnlock size={28} className="text-gray-400" />
                )}
              </div>
            </div>
